...
def fetch_data() -> pl.DataFrame:
    ...
    products_data = _fetch_all(
        ...
        "cpu, cpu_gen, memory, ssd, hdd, os, is_active, source, search_query",
    )
    ...
    price_df = pl.DataFrame(price_data, infer_schema_length=None).with_columns([
        pl.col("price").cast(pl.Int64, strict=False),
        pl.col("scraped_at").str.to_datetime(strict=False, time_unit="us", time_zone="UTC"),
    ])

    # Join price history → products; rename ambiguous columns from products side
    df = price_df.join(
        products_df.rename({
            "id": "product_id",
        ...
        for src in sorted(sources_present):
            ...
            trend = (
                ...
                .agg([
                    ...
                    pl.col("price").count().alias("listing_count"),
                ])
                .sort("run_date")
...
def render_listings_table(df: pl.DataFrame):
    ...
    cols = ["item_name", "price", "cpu", "memory", "ssd", "shop_name", "scraped_at", "item_url", "source"]
    available = [c for c in cols if c in df.columns]
    display = (
        df.select(available)
        .sort("price", descending=False)
        .with_columns(
            pl.col("price").map_elements(lambda x: f"¥{x:,}" if x else "—", return_dtype=pl.Utf8).alias("Price"),
    ...
    for row in display.iter_rows(named=True):
        ...
        <tr style="border-bottom:1px solid #1a1a1a;">
          <td style="max-width:300px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; padding:0.4rem 0.75rem;">{row.get('item_name') or '—'}</td>
          ...
          <td style="white-space:nowrap; padding:0.4rem 0.75rem;">{row.get('shop_name') or '—'}</td>
          <td style="font-family:'IBM Plex Mono',monospace; white-space:nowrap; padding:0.4rem 0.75rem;">{row.get('Scraped') or '—'}</td>
...
def render_all_models_trend(df: pl.DataFrame):
    ...
    for model, query in MODEL_QUERY_MAP.items():
        ...
        model_df = df.filter(pl.col("model") == query)
...
# Drop nulls on price
filtered_df = filtered_df.filter(pl.col("price").is_not_null())

# ── Combined all-models trend ────────────────────────────────────────────────────
st.markdown('<div class="model-title">All models price trend モデルごとの価格トレンド</div>',
            unsafe_allow_html=True)
