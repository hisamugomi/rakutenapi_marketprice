    ...
    async with async_playwright() as p:
        ...
        for item in item_containers:
            try:
                ...
                logger.exception("Failed to parse item in %s — skipping", search_key)
...
def run_pckoubou_scraper() -> list[dict]:
    ...
    for i, (key, url) in enumerate(SEARCH_URLS.items()):
        if i > 0:
            logger.info("[pckoubou] Waiting 10s before next URL")
            time.sleep(10)
        logger.info("[pckoubou] Scraping %s — %s", key, url)
        try:
            results = asyncio.run(scrape_pckoubou(key, url))
        except Exception:
            logger.exception("[pckoubou] scrape_pckoubou failed for %s", key)
            continue
        logger.info("[pckoubou] %s: %d items scraped", key, len(results))
