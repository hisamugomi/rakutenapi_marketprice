...
def _upsert_batch(
    ...
    logger.info("Upserted %d products", len(products_data))
...
def run_scraper() -> None:
    ...
    for query in QUERIES:
        logger.info("[rakuten] query=%s", query)
        raw = fetch_rakuten_items(query)
        if raw.is_empty():
            logger.warning("[rakuten] query=%s — no results", query)
            continue
        ...
        try:
            ...
            logger.exception("[rakuten] query=%s — upsert failed", query)

    # ── PC Koubou ────────────────────────────────────────────────────────────
    logger.info("[pckoubou] Starting scrape")
    ...
    if pckoubou_data:
        try:
            ...
            logger.exception("[pckoubou] upsert failed")

    # ── Sofmap ────────────────────────────────────────────────────────────────
    logger.info("[sofmap] Starting scrape")
    ...
    if sofmap_data:
        try:
            ...
            logger.exception("[sofmap] upsert failed")
